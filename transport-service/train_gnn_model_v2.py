"""
Train GNN Model on Transport Data (v2 - Simplified)

This script trains the GNN model using only:
...
The model learns to predict service ratings (0-1 scale) based on:
...
class TransportGNN(nn.Module):
    """GNN model for transport service rating prediction."""
...
def create_edge_features(edges_df):
    ...
    # For each edge, create feature vector
    for idx, edge in edges_df.iterrows():
        ...
        distance_norm = distance / 500  # Normalize by max distance
        duration_norm = duration / 600  # Normalize by max duration (in minutes)
        fare_norm = fare / 3000  # Normalize by max fare
        
        # Create feature vector: [mode, distance, duration, fare, is_active]
        features = np.array([
            mode_encoded / (len(mode_encoder.classes_) - 1),  # Normalize mode encoding
            distance_norm,
            duration_norm,
            fare_norm,
            float(is_active)
        ...
        # Generate synthetic target labels based on service quality heuristic
        # Higher rating for: active services, reasonable fare/duration ratio
        rating = (
            0.4 * float(is_active) +  # Is service active?
            0.3 * (1 - min(fare_norm, 1.0)) +  # Lower fare is better
            0.3 * (1 - min(duration_norm, 1.0))  # Shorter duration is better
        )
    ...
    print(f"  ✓ Target rating range: [{target_labels.min():.3f}, {target_labels.max():.3f}]")
...
def main():
    ...
    print("=" * 70)
    print("🚂 TRANSPORT SERVICE GNN MODEL TRAINING (v2)")
    print("=" * 70)
    print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    
    # Step 1: Load data
    nodes_df, edges_df = load_and_prepare_data("data")
    
    # Step 2: Create features
    node_features, type_encoder, region_encoder, coords_scaler = create_node_features(nodes_df)
    edge_features, target_labels, mode_encoder = create_edge_features(edges_df)
